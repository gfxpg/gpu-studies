...
impl Renderer {
    ...
    pub fn instantiate_with_texture(&self, texture: &web_sys::ImageBitmap) -> Result<(), JsValue> {
        ...
        let texcoords_array = interop::get_float32array(&self.model.texcoords)?;

        gl::load_attrib(&self.ctx, &self.program, "a_position", &vertices_array, 3, WebGl2RenderingContext::FLOAT, false)?;
        gl::load_attrib(&self.ctx, &self.program, "a_texcoord", &texcoords_array, 2, WebGl2RenderingContext::FLOAT, false)?;

        gl::load_texture_2d(&self.ctx, texture)?;
